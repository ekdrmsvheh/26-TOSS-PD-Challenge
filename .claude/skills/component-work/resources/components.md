...
- CardPageLayout: 카드(흰 박스, CardContainer) 기반 화면 공통 콘텐츠 셸. max-width 1280px·좌우 40px/상단 40px 패딩, title이 있으면 타이틀-카드 갭 20px. left가 있으면 좌(1개 카드)+우(카드 스택) 2컬럼 분할(컬럼 갭 20px, 우측 스택 갭 12px), 없으면 children을 단일 컬럼으로 렌더링 (`components/layout/CardPageLayout.jsx`)
...
- ReviewRequestSentView: 밋핏의 "일정 검토 요청 발송" 화면(플로우 3단계, Figma node-id 344:50380 계열 완료 화면 포맷 기준). CardPageLayout 단일 컬럼 모드(560px, 상단 여백 확보)로 구성 — 48px 체크 아이콘 + 안내 문구 + "응답 현황 보기" CTA만 제공하는 전환용 확인 화면. MeetingConfirmedView와 아이콘·타이포·여백 포맷을 공유한다 (`components/templates/ReviewRequestSentView.jsx`)
...
- MeetingConfirmedView: 밋핏의 "미팅 확정" 화면(플로우 6단계, Figma node-id 344:50380 기준). CardPageLayout 단일 컬럼 모드(560px, 상단 여백 확보)로 구성 — 48px 체크 아이콘 + "확정 일정" 카드(시작~종료 시간 범위 강조 + 회의 목적, elevation 카드)와 "참석자" 카드(포트레이트 아바타 + 이름·팀 + 참석/불참 2단 배지로 단순화, outlined 카드) + "완료" CTA (`components/templates/MeetingConfirmedView.jsx`)
