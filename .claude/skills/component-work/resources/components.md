...
- AttendeeRow: 미팅 참석자 행. 아바타/이름/역할/필수·권장·선택 레벨(정적 점 또는 onRoleLevelChange 전달 시 필수/선택 토글)/주최자 뱃지/삭제 버튼 (`components/input/AttendeeRow.jsx`)
...
- AttendeeSelectDialog: "참여 인원을 선택해 주세요" 모달. 주최자 카드(고정) + 검색 Autocomplete(directory에서 클릭해 추가, 기본 필수)로 채우는 참여자 목록(AttendeeRow 재활용, 필수/선택 토글·삭제) + 선택 완료 (`components/input/AttendeeSelectDialog.jsx`)
...
- AttendeeReviewView: 밋핏의 "참석자 일정 검토" 화면(참여자 역할 전용). CardPageLayout 단일 컬럼 모드(바깥 컨테이너 규격만 적용, 좌우 분할 없음)로 구성 — 본인이 필요한 이유 카드 + 후보 3개 카드(배지/메타 + 가능·비선호지만 가능·조정 필요·불가 4지 응답), 전체 응답 완료 시에만 제출 가능, 제출 후 완료 상태 표시 (`components/templates/AttendeeReviewView.jsx`)
